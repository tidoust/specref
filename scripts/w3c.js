 ...
 */
async function updateW3CRefs(sinceDate) {
...
if (since && !since.match(/^\d{4}(-\d\d(-\d\d)?)?$/)) {
    console.log(
...
if (start - (new Date(since)).getTime() > 365 * 24 * 3600 * 1000) {
    ...
    console.log("\x1b[41mBeware, update may take a while, possibly more than an hour!\x1b[0m");
}
