...
//
// `logF`, when given, is called with a message for every denied request
// instead of console.log.
function createIpFilter(bannedIPs, logF) {
    var options = { logLevel: "deny", trustProxy: true };
    if (logF) options.logF = logF;
    return IpFilter(bannedIPs, options);
}
